...
The upgrade procedure should be started on `August 02, 2021 at or around 11:35 UTC` by halting on block `517580`, with the new and exported genesis file of `pandora-5` having a genesis time of `2021-08-02T12:00:00Z`.
...
Not too many changes have occurred to the ixod software since the launch of pandora-4. These changes notably consist of an upgrade to the `bonds` module to enable reserve withdrawals, typically for funding purposes. Any existing bond in pandora-4 will have reserve withdrawals disabled.
...
__Note__: It is assumed you are currently operating a full-node running v0.6.0 of _ixo-blockchain_.

- The version/commit hash of ixo v0.8.0: `21e2c962e18220888d529bf156406260a321cf80` NEED TO UPDATE
- The upgrade height as agreed upon: **`517580`** NEED TO UPDATE


1. Verify you are currently running the correct version (v0.6.0) of the _ixo-blockchain_:
NEED TO UPDATE
   ```bash
   $ ixod version --long
   name: ixo
...
1. Verify you are currently running the correct version (v0.8.0) of _ixo_:
...
1. Migrate exported state from the current v1.5.0 version to the new v1.6.0 version. This will require running the migration Python script.
   ...
   ```bash
   $ python migrate_export_from_v0.6.0_to_v0.8.0
   ```

   **NOTE**: The `migrate_export_from_v0.6.0_to_v0.8.0` script takes an input genesis state and migrates it to a genesis file _genesis.json_ readable by ixo v0.8.0, and updating the genesis time and initial height.
...
This section lists all steps that the [Python migration script](./scripts/migrate_export_from_v0.6.0_to_v0.8.0.py) performs.
